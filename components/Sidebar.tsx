...
const navItems = [
  { name: "Dashboard", href: "/", icon: (active: boolean) => (
...
export default function Sidebar() {
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`group flex items-center px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 border-l-2 ${
                  active
  ...
  return (
    <>
      ...
      <div className="lg:hidden flex items-center justify-between p-4 glassmorphic sticky top-0 z-40 w-full">
      ...
      {isOpen && (
        <div
          onClick={toggleSidebar}
          className="lg:hidden fixed inset-0 bg-slate-950/40 backdrop-blur-sm z-30 transition-opacity duration-300"
        />
      )}

      {/* Mobile Navigation Drawer */}
      <div
        className={`lg:hidden fixed top-0 bottom-0 left-0 z-40 w-72 glassmorphic-card p-6 transition-transform duration-300 ease-in-out ${
      ...
      <div className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 lg:z-30 glassmorphic-card p-6 m-4 h-[calc(100vh-2rem)] rounded-2xl">
        <SidebarContent />
      </div>
    </>
  );
